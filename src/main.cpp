...
void sendMessageMS(int ms) {
    for(int i = 0; i < 100; i++) {
        delay(ms);
        String bStr("robot15/sendMessage:1");
        publishMQTT(bStr);
    }
    return;
}
...
void sendLongMessageMS(int ms) {
    for(int i = 0; i < 100; i++) {
        delay(ms);
        String bStr("robot15/sendLongMessage:Hello there! I hope you are enjoying working for the RBE department. From, Ashe Andrews");
        publishMQTT(bStr);
    }
    return;
}